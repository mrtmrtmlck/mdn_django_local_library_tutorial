...
class LoanedBookInstanceByUserListViewTest(TestCase):
    ...
    def test_pages_ordered_by_due_date(self):
        for book_copy in BookInstance.objects.all():
            book_copy.status = 'o'
            book_copy.save()
        ...
        last_date = 0
        for book_copy in response.context['bookinstance_list']:
            if last_date == 0:
                last_date = book_copy.due_back
            else:
                self.assertTrue(last_date <= book_copy.due_back)
                last_date = book_copy.due_back
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_form_renewal_date_initially_has_date_three_weeks_in_future(self):
        ...
        date_3_weeks_in_future = self.test_bookinstance1.due_back + datetime.timedelta(weeks=3)
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_initial_date_of_birth_is_today(self):
        self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.context['form'].initial['date_of_birth'], datetime.date.today())
